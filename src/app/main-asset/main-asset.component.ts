import { Component, OnInit, Renderer2, TemplateRef, ViewChild, ElementRef, OnDestroy, ChangeDetectorRef } from '@angular/core';
...
export class MainAssetComponent implements OnInit, OnDestroy {
    ...
    this.walletService.walletChange$
      .subscribe(wallets => {
        this.selectedWalletData = wallets
          .find(s => s['symbol'] === this.selectedWallet);
        this.selectedSymbolData = this.symbolService.symbols
          .find(s => s['symbol'] === this.selectedWallet);
  ...
  loadBackground() {
    ...
    this.renderer.setStyle(this.bodyEl, 'background-size', '150% 150%');
  }
  ...
  selectSymbol(s, event) {
    ...
    if (event) {
      ...
      setTimeout(function () {
        ...
        if ($('.sidenav ul li[data-symbol=\'' + s.symbol + '\']').offset()) {
  ...
  loadTransaction() {
    ...
    return new Promise((resolve, reject) => {
      this.transactionService.getTransactions(this.selectedWallet, this.page, 10)
        .subscribe((r: any) => {
  ...
  toggleDefault() {
    this.walletService.toggleDefault(this.selectedWalletData.symbol, !this.selectedWalletData.isDefault)
      .subscribe((r: any) => {
        ...
        this.alertService.push('success', `${this.selectedWalletData.title} wallet is ${r.isDefault ? 'active' : 'inactive'} `);
      });
  }
  ...
  slide(forward: boolean) {
    let index = this.symbolsDisplayed.findIndex(w => w.symbol === this.selectedWalletData.symbol);
    if (forward) {
      index ++;
    } else {
      index --;
    }
    if (index >= 0 && index < this.symbolsDisplayed.length) {
      this.forward = forward ? 1 : 2;
      this.selectSymbol(this.symbolsDisplayed[index], null);
    }
  ...
  getTextOfTransaction(transType) {
    switch (transType) {
      case 'exchangeReceive':
        return 'Exchange';
      case 'exchangeSend':
  ...
  getWallets() {
    return this.wallets.filter(w => w.symbol !== this.selectedWallet).sort((a, b) => {
      if (a.symbolData.price > b.symbolData.price) {
        return -1;
      } else if (a.symbolData.price < b.symbolData.price) {
        return 1;
      }
      return 0;
    });
  ...
  getMegaFormattedValue(val) {
    return (val / 1000000).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') + 'M';
  }

  get24ChangePrice() {
    return (this.selectedWalletData.symbolData.price * (100 + this.selectedWalletData.symbolData.percentChange24h) / 100).toFixed(2);
  }

  onScroll(event) {
    if (this.scrollStarted && this.page <= this.pages) {
      this.loadTransaction();
    }
  }
